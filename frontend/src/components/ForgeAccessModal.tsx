...
export default function ForgeAccessModal({ open, onClose, defaultTab }: Props) {
  ...
  const providers = [
    {
      id: "google", label: "Google", comingSoon: false, onClick: handleGoogleClick,
    ...
    {
      id: "microsoft", label: "Microsoft", comingSoon: false,
        ...
        {screen === "grid" && (
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
              {providers.map((p) => (
                <button key={p.id} type="button" onClick={p.comingSoon ? undefined : p.onClick}
                  ...
                  onMouseEnter={(e) => { if (!p.comingSoon) (e.currentTarget as HTMLButtonElement).style.background = "rgba(60,222,106,0.08)"; }}
                  onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = "rgba(255,255,255,0.04)"; }}
                >
                  <div>{p.icon}</div>
                  <div style={{ fontSize: 13, fontWeight: 800, color: "#fff" }}>{p.label}</div>
                  {p.comingSoon && (
                    <div style={{ fontSize: 9, fontWeight: 700, color: "rgba(255,255,255,0.4)", letterSpacing: 0.5 }}>COMING SOON</div>
                  )}
                </button>
              ))}
            </div>
